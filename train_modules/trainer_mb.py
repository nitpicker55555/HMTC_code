...
class Trainer(object):
    ...
    def run(self, data_loader, epoch, stage, mode='TRAIN'):
        ...
        def memory_generate(batch_examples,labels):
            ...
            if memory_bank == None:
                ...
                # print(memory_bank.size(),"memory_bank.size()[0]")
                # print(memory_labels.size(),"memory_labels")
        ...
        for batch in tqdm.tqdm(data_loader):
            ...
            label_loss = self.SampleContrastiveloss(label_information.to(self.device), label_loss_pos_mask.to(self.device), torch.tensor(self.ac_matrix).to(
                                                                                ...
                                                                                self.device))
            # sample_loss = self.SampleContrastiveloss(label_information, sample_loss_pos_mask, torch.tensor(self.ac_matrix).to(
            #                                                                     self.device))
    ...
    def SampleContrastiveloss(self,features, positive_mask, cost_matrix, memory_features=None, temperature=0.07):
        ...
        # print(batch_size)
        # print(features.shape, "contrast_feature")
        # anchor_norm = torch.norm(features, dim=1)
        # features = features / (anchor_norm.unsqueeze(1))
        ...
        #     1,
        #     torch.arange(batch_size * num_labels).view(-1, 1),
        #     0
        # )
